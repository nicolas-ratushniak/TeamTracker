...
public class TeamUpdateDto
{
    ...
    [RegularExpression(@"^[A-Z]([\s\-]?[a-zA-Z\d])*$", ErrorMessage =
        "Name field can only contain letters, digits and \"-\". It should start with a capital letter and end with no space")]
    ...
    [RegularExpression(@"^[A-Z]([\s\-]?[a-zA-Z])*$", ErrorMessage =
            "Origin City can only contain letters and \"-\". It should start with a capital letter and end with no space")]
